...
import PopupContent from 'src/components/PopupContent.vue'
...
export default {
  ...
  data () {
    return {
      accessToken: 'pk.eyJ1IjoibmthbjAwMSIsImEiOiJja2M1am9rM3EwNDZpMnZsajlkdTltb3lyIn0._-c81YPoAJ7CvdgSEpVSSg',
      options: {
        container: 'map',
  ...
  mounted () {
    for (var i = 0; i < (this.parks.features).length; i++) {
      delete this.parks.features[i].properties['Name']

      let temp = this.parks.features[i].properties['Description']
      this.parks.features[i].properties['description'] = temp

      delete this.parks.features[i].properties['Description']
      this.parks.features[i].properties['icon'] = 'park'
    }
  }
}
</script>
